...
OUTPUT_ROOT = Path(r"G:/A_Veg_Model/Adaption")
...
def load_feature_arrays(
    ...
    inun_duration_template: str = INUNDATION_DURATION_TEMPLATE,
    inun_mean_wl_template: str = INUNDATION_MEAN_WL_TEMPLATE,
    climate_root: str = CLIMATE_ROOT,
) -> Dict[str, np.ndarray]:
    """Load feature rasters for a single year (excluding MAVI)."""
    duration_array, _, _ = load_raster(Path(inun_duration_template.format(year=year)))
    mean_wl_array, _, _ = load_raster(Path(inun_mean_wl_template.format(year=year)))
    ...
    for var in CLIMATE_VARIABLES:
        if var not in feature_keys:
            continue
        climate_path = Path(
            CLIMATE_TEMPLATE.format(root=climate_root, var=var, year=year-20)
        )
...
def export_features_to_csv(years: List[int]) -> None:
    """Flatten yearly feature rasters into per-year CSV files (MAVI excluded).
    ...
    for year in tqdm(missing_years, desc="Exporting rasters to CSV"):
        ...
        # Only write feature columns (exclude MAVI)
        for key, array in data.items():
            if key == "MAVI":
                continue
...
def _load_array_from_csv(year: int, column: str, shape: Tuple[int, int]) -> np.ndarray:
    ...
    if column == "MAVI":
        return _load_mavi_array(year, shape)
...
def _load_mavi_array(year: int, shape: Tuple[int, int]) -> np.ndarray:
    """Load the observed MAVI raster for a given year and return as array."""

    mavi_path = Path(BASE_MAVI_TEMPLATE.format(year=year))
    mavi_array, _, _ = load_raster(mavi_path)
    if mavi_array.shape != shape:
        raise ValueError(f"MAVI raster shape {mavi_array.shape} does not match reference {shape}")
    return mavi_array
...
def compute_period_mean_from_csv(years: List[int], feature_keys: List[str]) -> pd.DataFrame:
    ...
    required_cols = ["row", "col", *feature_keys]
    ...
    for year in years:
        ...
        # Add MAVI from raster to align with feature rows
        mavi_array = _load_mavi_array(year, (df.index.max()[0] + 1, df.index.max()[1] + 1))
        df["MAVI"] = mavi_array[df.index.get_level_values(0), df.index.get_level_values(1)]
...
def build_full_sequences(
    ...
    for t, year in enumerate(years):
        csv_path = FEATURE_CSV_DIR / f"mavi_features_{year}.csv"
        df = pd.read_csv(csv_path, usecols=["row", "col", *feature_columns])
        ...
        mavi_array = _load_mavi_array(year, reference_shape)
        mavi_vals = mavi_array[rows_arr[valid], cols_arr[valid]]
...
def collect_valid_coords(
    ...
    """Collect valid pixel coordinates (row, col) present in feature CSVs."""
    ...
    for year in years:
        ...
        df = pd.read_csv(csv_path, usecols=["row", "col"])
        valid_coords.update(zip(df["row"].astype(int), df["col"].astype(int)))
...
def collect_coords_from_features(
    years: List[int],
    feature_keys: List[str],
    ...
    """Collect valid coordinates directly from feature rasters (no MAVI required)."""
    ...
    for year in years:
        ...
        if shape is None:
            first_arr = next(iter(features.values()))
            shape = first_arr.shape
        year_mask = np.ones(shape, dtype=bool)
        for arr in features.values():
            year_mask &= ~np.isnan(arr)
        valid_mask = year_mask if valid_mask is None else (valid_mask | year_mask)

    if valid_mask is None or shape is None:
        raise ValueError("No features found to derive valid coordinates.")

    rows, cols = np.nonzero(valid_mask)
    coord_list = list(zip(rows, cols))
    pixel_mask = valid_mask.reshape(-1)
    return np.array(coord_list, dtype=np.int32), pixel_mask, shape
...
def hyperparameter_search(
    ...
    configs = [
        {"hidden_size": 192, "num_layers": 2, "dropout": 0.4, "learning_rate": 6e-4, "weight_decay": 2e-4},
    ]
...
def main2() -> None:
    """Simulate post-2003 MAVI under alternate hydrologic scenarios without observed MAVI."""

    post_years = list(range(2020, 2040))
    all_years = list(range(2003, 2040))

    scenario_configs = [
        {
            "name": "H1_case1",
            "inun_root": r"G:/A_Veg_Model/TGP_future_influence/H1",
        },
        {
            "name": "H1_case2",
            "inun_root": r"G:/A_Veg_Model/TGP_future_influence/H2",
        },
    ]

    scenario_output_root = Path(r"G:/A_Veg_Model/TGP_future_influence/A1")
    ...
    for scenario in scenario_configs:
        ...
        # Derive reference grid from the scenario inundation duration raster.
        first_duration, geotransform, projection = load_raster(Path(duration_template.format(year=post_years[0])))
        reference_shape = first_duration.shape

        coords, pixel_mask, reference_shape = collect_coords_from_features(
            post_years,
            feature_keys,
            duration_template,
            mean_wl_template,
        ...
        scenario_output_dir = scenario_output_root / f"predicted_mavi_{name}"
        yearly_metrics = predict_full_sequence(
            model,
            seqs_norm,
            post_years,
        ...
        yearly_metrics_df = pd.DataFrame(yearly_metrics)
        yearly_metrics_csv = scenario_output_root / f"yearly_metrics_{name}.csv"
        yearly_metrics_df.to_csv(yearly_metrics_csv, index=False)
        ...
        summary_csv = scenario_output_root / f"summary_metrics_{name}.csv"
        summary_df.to_csv(summary_csv, index=False)
